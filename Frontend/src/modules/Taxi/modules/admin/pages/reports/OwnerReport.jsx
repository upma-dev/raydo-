...
const OwnerReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const locationsRes = await adminService.getServiceLocations();
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      triggerFileDownload(response, `owner_report_${Date.now()}`, filters.file_format);
    } catch (err) {
  ...
  const isFormValid = filters.service_location_id && filters.approval_status && filters.date_option && filters.file_format && (filters.date_option !== 'range' || (filters.from_date && filters.to_date));
    ...
    <div className="min-h-screen bg-[#F9FAFB] p-6 lg:p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 text-xs text-gray-400 mb-3 font-medium">
          ...
          <span className="text-gray-600 font-semibold italic">Owner Report</span>
        </div>
        <div className="flex items-center justify-between border-b border-gray-100 pb-5">
          <h1 className="text-2xl font-bold text-[#334155] tracking-tight uppercase">Owner Report</h1>
          <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
            ...
            <div className="space-y-1">
              <label className={labelClass}>
                Select Service Location <span className="text-rose-500">*</span>
              </label>
